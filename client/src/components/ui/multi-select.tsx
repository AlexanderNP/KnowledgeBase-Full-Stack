...
/**
 * Option interface for MultiSelectBase component
 */
interface MultiSelectBaseOption {
 ...
 */
interface MultiSelectBaseGroup {
  /** Group heading */
  heading: string;
  /** Options in this group */
  options: MultiSelectBaseOption[];
}

/**
 * Props for MultiSelectBase component
 */
interface MultiSelectBaseProps
   ...
   */
  options: MultiSelectBaseOption[] | MultiSelectBaseGroup[];
  /**
 ...
 */
export interface MultiSelectBaseRef {
  /**
...
export const MultiSelectBase = React.forwardRef<MultiSelectBaseRef, MultiSelectBaseProps>(
  (
    {
      options,
      onValueChange,
    ...
    const isGroupedOptions = React.useCallback(
      (opts: MultiSelectBaseOption[] | MultiSelectBaseGroup[]): opts is MultiSelectBaseGroup[] => {
    ...
    const getAllOptions = React.useCallback((): MultiSelectBaseOption[] => {
      if (options.length === 0) return [];
      let allOptions: MultiSelectBaseOption[];
      ...
      const uniqueOptions: MultiSelectBaseOption[] = [];
      ...
      if (process.env.NODE_ENV === "development" && duplicates.length > 0) {
        ...
        console.warn(
          `MultiSelectBase: Duplicate option values ${action}: ${duplicates.join(", ")}. ` +
            `${
              deduplicateOptions
    ...
    const getOptionByValue = React.useCallback(
      (value: string): MultiSelectBaseOption | undefined => {
        const option = getAllOptions().find((option) => option.value === value);
        if (!option && process.env.NODE_ENV === "development") {
          console.warn(`MultiSelectBase: Option with value "${value}" not found in options list`);
        }
        return option;
      },
      [getAllOptions],
...
interface MultiSelectProps<T> extends Omit<MultiSelectBaseProps, "options"> {
  options: T[];
  optionLabel?: keyof T;
  optionValue?: keyof T;
}
...
type MultiSelectComponent = <T extends Record<string, any>>(
  props: MultiSelectProps<T> & React.RefAttributes<MultiSelectBaseRef>,
...
export type { MultiSelectBaseOption, MultiSelectBaseGroup, MultiSelectBaseProps, MultiSelectProps };
